...
def generate_dataset(args, data, labelled: bool = True):
    ...
    # 理解　ここで Eが9になっている．なぜか <- 一つのsentenceの中で最大9このentitiesがあるから
    feature_dict = {}
    for k, v in data.items():
        if k in [
            ...
            feature_dict[k] = nn.utils.rnn.pad_sequence(
                v, batch_first=True, padding_value=0
            )
        elif k in ["position"]:
            feature_dict[k] = nn.utils.rnn.pad_sequence(
...
def convert_examples_to_features_for_earningcall(examples, labeled):
    ...
    for idx in range(len(examples["company_name"])):
        examples["label_avg3days"] += [
            label_map[str(int(examples["y_avg3days"][idx] >= 0))]
        ]
        examples["label_avg7days"] += [
            label_map[str(int(examples["y_avg7days"][idx] >= 0))]
        ]
        examples["label_avg15days"] += [
            label_map[str(int(examples["y_avg15days"][idx] >= 0))]
        ]
        examples["label_avg30days"] += [
            label_map[str(int(examples["y_avg30days"][idx] >= 0))]
        ]

        examples["label_single3days"] += [
            label_map[str(int(examples["y_single3days"][idx] >= 0))]
        ]
        examples["label_single7days"] += [
            label_map[str(int(examples["y_single7days"][idx] >= 0))]
        ]
        examples["label_single15days"] += [
            label_map[str(int(examples["y_single15days"][idx] >= 0))]
        ]
        examples["label_single30days"] += [
            label_map[str(int(examples["y_single30days"][idx] >= 0))]
        ]
...
def load_examples(
    ...
    *_,
    ...
    seed: int = 42,
    split_examples_evenly: bool = False,
):
    ...
    if task == "EarningCall":
        # Processorsを初期化するときにfile not found errorが出ている
    ...
    if set_type == TRAIN_SET:
        # 修正 selfしか受け取らないみたいだから，data_dirを削除
...
class EarningCallProcessor(DataProcessor):
    ...
    def __init__(self, args, data_dir):
        ...
        processed_ptm_path = os.path.join(data_dir, "processed_dataset_for_kept")
        ...
        for ptm_dataset_name in tqdm(processed_ptm_list):
            sizes_list += [
                (
                    ptm_dataset_name,
                    os.path.getsize(processed_ptm_path + "/" + ptm_dataset_name),
                )
            ]
        ...
        for ptm_dataset_name, _ in tqdm(ptm_dataset_list):
            ...
            self.ptm_dataset += pickle.load(
                open(processed_ptm_path + "/" + ptm_dataset_name, "rb")
            )
    ...
    def generate_padded_token_embedding(self, max_sent=520):
        ...
        for k in tqdm(token_embedding.keys()):
            ...
            if max_sent > num_sent:
                padded_out_for_on_text = torch.cat(
                    [
                        out_for_on_text,
                        torch.zeros(max_sent - num_sent, sent_max_len, ptm_size),
                    ],
                    dim=0,
                )
            else:
    ...
    def PTMData(self):
        ...
        for item in tqdm(self.ptm_dataset):
            ...
            rel_nd_input_ids = []
            tail_nd_input_ids = []
            # 理解 entitiesに対して，たまに複数entitiesが文章に含まれている
            for ent in item["entities"]:
                ...
                # 理解 entityのposition．何におけるpositionなのかは今のところあまりわからない
                position += [torch.tensor(ent["position"])]
                for trp, desc in zip(ent["triples"], ent["desc"]):
                    # 理解 Figure1でいうところの[EBIT [SEP] earnings before interest and taxes]みたいなデータを作成している
                    # for each sentence:
                    #   for each entry:
                    #    for each triple:　←このレベルに対してデータセット作成
                    head_name_desc = self.args.tokenizer(
                        ent["name"]
                        + " "
                        + self.args.tokenizer.sep_token
                        + " "
                        + ent["description"],
                        padding="max_length",
                        max_length=self.args.entity_max_length,
                        truncation=True,
                        return_tensors="pt",
                    )
                    one_ent_head_nd_input_ids += [head_name_desc["input_ids"]]

                    rel_name_desc = self.args.tokenizer(
                        trp[1] + " " + self.args.tokenizer.sep_token + " " + desc[1],
                        padding="max_length",
                        max_length=self.args.entity_max_length,
                        truncation=True,
                        return_tensors="pt",
                    ...
                    tail_name_desc = self.args.tokenizer(
                        trp[2] + " " + self.args.tokenizer.sep_token + " " + desc[2],
                        padding="max_length",
                        max_length=self.args.entity_max_length,
                        truncation=True,
                        return_tensors="pt",
                ...
                # 理解　ここでtriplesの数を6に揃えている．
                head_nd_input_ids += [
                    F.pad(
                        nn.utils.rnn.pad_sequence(
                            one_ent_head_nd_input_ids, batch_first=True, padding_value=0
                        ).squeeze(1),
                        (0, 0, 0, 6 - len(ent["triples"])),
                        "constant",
                        0,
                    )
                ]
                rel_nd_input_ids += [
                    F.pad(
                        nn.utils.rnn.pad_sequence(
                            one_ent_rel_nd_input_ids, batch_first=True, padding_value=0
                        ).squeeze(1),
                        (0, 0, 0, 6 - len(ent["triples"])),
                        "constant",
                        0,
                ...
                tail_nd_input_ids += [
                    F.pad(
                        nn.utils.rnn.pad_sequence(
                            one_ent_tail_nd_input_ids, batch_first=True, padding_value=0
                        ).squeeze(1),
                        (0, 0, 0, 6 - len(ent["triples"])),
                        "constant",
                        0,
            ...
            if (
                head_nd_input_ids == []
                or rel_nd_input_ids == []
                or tail_nd_input_ids == []
            ):
                continue

            position = nn.utils.rnn.pad_sequence(
                position, batch_first=True, padding_value=-1
            )
            # 理解　positionの最大長は2になる <- e-commerceなどは3になるので，これらは除外される．EPSなどは大丈夫なんだろうな
            ...
            mlm_input = self.args.tokenizer(
                item["mlm_sent"],
                padding="max_length",
                max_length=self.args.ratio_max_length,
                truncation=True,
                is_split_into_words=True,
                return_tensors="pt",
            )
            lb_mlm_index = self.args.tokenizer.convert_tokens_to_ids(item["mlm_labels"])
            # 修正　mask idはbert base uncasedでは103である．なんで5を使っているんだろう．．．tokenizerをどこか知らないところで自作しているのか，，，？
            # b_mask_posi, _ = (mlm_input["input_ids"] == 5).nonzero(
            #     as_tuple=True
            # )  # 5 is mask id
            # lb_bidx = (torch.tensor(lb_mlm_index) != 0).nonzero(as_tuple=True)[0]
            # 理解　b_mask_posiはrowsで_がcolumnsだから，これ自身に意味はないが，len(b_mask_posi)はmaskの数を表している．
            b_mask_posi, _ = (mlm_input["input_ids"] == 103).nonzero(
                as_tuple=True
            )  # <- in this case, 103 is mask id
            lb_bidx = (torch.tensor(lb_mlm_index) != 0).nonzero(as_tuple=True)[0]

            if len(b_mask_posi) != len(lb_bidx):
                # 理解　maskの数とlabelの数が一致しない場合は，このデータを除外する操作
                # 比較するときにTokenizersでtruncateされたmlm_inputと，truncateされていないitem["mlm_labels"]を使用しているので
                # Truncateされているような長い文章は全て除外されてしまっている←これって意図されているんだろうか？論文中には記載がない
            ...
            out_dict["head_nd_input_ids"] += [
                nn.utils.rnn.pad_sequence(
                    head_nd_input_ids, batch_first=True, padding_value=0
                )
            ]
            out_dict["rel_nd_input_ids"] += [
                nn.utils.rnn.pad_sequence(
                    rel_nd_input_ids, batch_first=True, padding_value=0
                )
            ]
            out_dict["tail_nd_input_ids"] += [
                nn.utils.rnn.pad_sequence(
                    tail_nd_input_ids, batch_first=True, padding_value=0
                )
            ]
            ...
            input_is_ratio = self.args.tokenizer(
                item["raw_sent"],
                padding="max_length",
                max_length=self.args.ratio_max_length,
                truncation=True,
    ...
    def _create_examples(self):
        ...
        # 修正　← これ絶対いらない．NEW tokensがAutoTokenizerで追加されているのに，ここでtokenizerを変更したら
        # おかしなことが起こる
